...
try {
  const scrapedRaw = fs.readFileSync(path.join(process.cwd(), 'trip_data_scraped_costsaver.json'), 'utf8');
  ...
  console.warn('⚠️  trip_data_scraped_costsaver.json not found or invalid. Using defaults.');
}
    ...
    for (const contentBlock of contentBlocks) {
      ...
      for (const dep of departures) {
        ...
        for (const sr of sellingRegions) {
          ...
          if (startDate && endDate) { return daysInclusive(startDate, endDate); }
        }
      }
    }
...
function transformTourData(tour) {
  ...
  // Attach scraped data if available
  const scrapedData = scrapedDataMap.get(Number(tour.id)) || {};
  trip.avg_rating = safeNum(scrapedData.avg_rating);
  trip.total_reviews = safeNum(scrapedData.total_reviews);
  trip.activity_level = scrapedData.activity_level || null;
  ...
  for (const option of tour.tourOptions || []) {
    for (const season of option.seasons || []) {
      ...
      if (!trip.region && seasonContentContinent) { trip.region = seasonContentContinent; }
...
async function saveTripsToJSON(transformedTrips) {
  console.log(`💾 Processing ${transformedTrips.length} trips for trips JSON (CostSaver)...`);
  const outputPath = path.join(process.cwd(), 'trips_costsaver.json');
  try {
    ...
    console.error('❌ Error saving trips JSON (CostSaver):', error);
...
async function processTripsData() {
  try {
    console.log('🚀 Starting trips data processing (CostSaver)...');
    logMemory("Start of costsaver/trips.js");
    ...
    return new Promise((resolve, reject) => {
      ...
      parser.on('end', async () => {
        logMemory("After streaming and processing tours (CostSaver)");
        ...
        logMemory("After saving trips JSON (CostSaver)");
        console.log('✅ Trips data processing completed (CostSaver)!');
    ...
    console.error('❌ Error processing trips data (CostSaver):', error);
    process.exit(1);
  }
}

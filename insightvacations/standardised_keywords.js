...
const mergedData = JSON.parse(fs.readFileSync("trips_with_reviews_insight.json", "utf-8"));
...
function saveCheckpoint(processedTrips, currentIndex) {
  ...
  fs.writeFileSync('keywords_checkpoint_insight.json', JSON.stringify(checkpoint, null, 2));
}

// --- Load checkpoint if exists ---
function loadCheckpoint() {
  try {
    if (fs.existsSync('keywords_checkpoint_insight.json')) {
      const checkpoint = JSON.parse(fs.readFileSync('keywords_checkpoint_insight.json', 'utf8'));
...
async function main() {
  console.log(`🚀 Starting keyword enrichment for ${allTrips.length} Insight trips (VPS-optimized)...`);
  ...
  // Save final output
  const outputFile = "trips_with_standardised_keywords_insight.json";
  fs.writeFileSync(outputFile, JSON.stringify(finalTrips, null, 2));

  // Clean up checkpoint file
  if (fs.existsSync('keywords_checkpoint_insight.json')) {
    fs.unlinkSync('keywords_checkpoint_insight.json');
  }

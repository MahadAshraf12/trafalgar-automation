...
function readTripsWithReviewsData() {
  const tripsFile = path.join(process.cwd(), 'trips_with_reviews_insight.json');
...
function readKeywordsData() {
  const keywordsFile = path.join(process.cwd(), 'trips_with_standardised_keywords_insight.json');
...
function saveFinalTrips(enhancedTrips) {
  const outputFile = path.join(process.cwd(), 'trips_enriched_final_insight.json');

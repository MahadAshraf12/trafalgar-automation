...
function readTripCodes() {
  const filePath = 'trip_codes_insight.json';
...
async function fetchFeefoReviews() {
  ...
  for (let i = 0; i < allTripCodes.length; i += BATCH_SIZE) {
    ...
    for (let page = 1; page <= PAGES_NEEDED; page++) {
      try {
        const baseUrl = 'https://www.insightvacations.com/en-us/feefo/getreviews';
        const params = {
          merchantId: 'insight-vacation',
        ...
        const response = await axios.get(baseUrl, {
          ...
          headers: {
            ...
            'Referer': 'https://www.insightvacations.com/'
          },
          timeout: 30000
        });
  ...
  const outputFile = 'feefo_reviews_insight.json';

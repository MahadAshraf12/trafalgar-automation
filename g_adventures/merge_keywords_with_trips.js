...
function readTripsWithReviewsData() {
  const tripsFile = path.join(process.cwd(), 'final_trips.json');
...
function readKeywordsData() {
  const keywordsFile = path.join(process.cwd(), 'trips_with_standardised_keywords_gadventures.json');
...
function mergeKeywordsWithTrips(trips, keywordsMap) {
  ...
  const enhancedTrips = trips.map(trip => {
    // Convert trip_id to string for consistent matching
    const tripIdKey = String(trip.id || trip.trip_id);
    ...
    // Add keywords to the trip object
    return {
      ...trip,

...
// --- STEP 1: Load final trips file ---
const mergedData = JSON.parse(fs.readFileSync("final_trips.json", "utf-8"));
...
Trip SKU: ${trip.trip_productline || trip.product_line || ""}
Description: ${trip.trip_description || trip.description || ""}
Details: ${JSON.stringify(getFirst10(trip.details || []), null, 2)}
Geography: ${JSON.stringify(trip.region || "", null, 2)}
Reviews: ${JSON.stringify(trip.reviews || {}, null, 2)}
...
`;
  ...
  for (let attempt = 1; attempt <= CONFIG.MAX_RETRIES; attempt++) {
    try {
      ...
      if (!Array.isArray(result.standardised_keywords)) {
        throw new Error('Invalid response structure from OpenAI');
      }

      // Ensure trip_id is set
      result.trip_id = result.trip_id || trip.trip_id || trip.id || "";

      console.log(`✅ Successfully processed trip ${result.trip_id} on attempt ${attempt}`);
      return result;
      ...
      if (attempt === CONFIG.MAX_RETRIES) {
        ...
        return {
          ...
          trip_sku: trip.trip_productline || trip.product_line || "",
...
function saveCheckpoint(processedTrips, currentIndex) {
  ...
  fs.writeFileSync('keywords_checkpoint_gadventures.json', JSON.stringify(checkpoint, null, 2));
}

// --- Load checkpoint if exists ---
function loadCheckpoint() {
  try {
    if (fs.existsSync('keywords_checkpoint_gadventures.json')) {
      const checkpoint = JSON.parse(fs.readFileSync('keywords_checkpoint_gadventures.json', 'utf8'));
...
async function main() {
  console.log(`🚀 Starting keyword enrichment for ${allTrips.length} G Adventures trips (VPS-optimized)...`);
  ...
  for (let i = startIndex; i < allTrips.length; i++) {
    ...
    try {
      ...
      console.log(`✅ Completed trip ${i + 1}/${allTrips.length}`);
  ...
  // Save final output
  const outputFile = "trips_with_standardised_keywords_gadventures.json";
  fs.writeFileSync(outputFile, JSON.stringify(finalTrips, null, 2));

  // Clean up checkpoint file
  if (fs.existsSync('keywords_checkpoint_gadventures.json')) {
    fs.unlinkSync('keywords_checkpoint_gadventures.json');
  }

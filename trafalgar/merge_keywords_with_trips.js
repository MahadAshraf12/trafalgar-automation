...
function readTripsWithReviewsData() {
  const tripsFile = path.join(process.cwd(), 'trips_with_reviews.json');
...
function readKeywordsData() {
  const keywordsFile = path.join(process.cwd(), 'trips_with_standardised_keywords.json');
...
function saveFinalTrips(enhancedTrips) {
  const outputFile = path.join(process.cwd(), 'trips_enriched_final.json');
  ...
  console.log(`✅ Saved ${enhancedTrips.length} enriched trips with reviews and keywords`);

...
function transformTourData(tour) {
  ...
  const ratingsData = JSON.parse(fs.readFileSync('trip_ratings_header.json', 'utf8'));
  const ratingObj = ratingsData.find(r => r.trip_id === tour.id);
  ...
  // Group sizes removed as requested
  ...
  // Add new fields from scraped data
  trip.service_level = 'Standard';
  trip.activity_level = ratingObj?.tweaked_icons?.activity_level || null;
  trip.avg_rating = ratingObj?.rating || null;
  trip.total_reviews = ratingObj?.review_count || null;
  trip.trip_provider = 'TRAFALGAR';
